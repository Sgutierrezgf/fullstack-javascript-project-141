...
/**
 * Compatible with Fastify 5 (avoids mixing async + callback styles).
 * Based on fastify-method-override.
 */
const methodOverride = async (fastify) => {
  ...
  const handleRedirect = async (req, reply) => {
    ...
    if (originalMethod === 'post' && allowMethods.has(method)) {
      ...
      if (!handler) {
        throw createError.NotFound(`Route ${_.toUpper(method)}:${url} not found`);
      }
...
export default fp(methodOverride, {
